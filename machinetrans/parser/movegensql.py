"""
    MoveGenSQL:  A class to generate English sentences 
...
class MoveGenSQL(WordGenSQL):
    """ A class to generate English sentences with verbs 
        of motion.  This is a subclass of WordGenSQL
        which handles database management and data generation.
    """
    def __init__(self):
        """ Initialize the class.
        """
        super(MoveGenSQL, self).__init__()
        return
    ...
    def makesentences(self, choice):
        ...
        if choice == 0:
            ...
            """    
            sub = SubVerb( self.conn, self.sentence["articles"], self.sentence["subjects"], self.sentence["motions"])    
            tmpstr = sub.compose() + ". "
            self.datastr += "\n" + tmpstr
        elif choice == 1:    
            ...
            """
            sub = SubVerb( self.conn, self.sentence["articles"], self.sentence["subjects"], self.sentence["motions"], None, self.sentence["advsmanner"])    
            obj = ArtObj(self.sentence["articles"], self.sentence["objects"], self.sentence["adjectives"], self.sentence["prepositions"])
            tmpstr = sub.compose() + " "
            tmpstr += obj.compose() + "."
            self.datastr += "\n" + tmpstr
            ...
            """ Generates the following:  A-ADJ-S-AV-V-A-ADJ-O
                where A = article, S = subject, V = verb, O = object, ADJ = adjective and AV = adverb.
            """
            sub = SubVerb( self.conn, self.sentence["articles"], self.sentence["subjects"], self.sentence["motions"], self.sentence["adjectives"], self.sentence["advsmanner"])    
            obj = ArtObj(self.sentence["articles"], self.sentence["objects"], self.sentence["adjectives"], self.sentence["prepositions"])
            tmpstr = sub.compose() + " "
            tmpstr += obj.compose() + "."
            self.datastr += "\n" + tmpstr
